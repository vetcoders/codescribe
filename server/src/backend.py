...
# ruff: noqa: E402 - imports must be after dotenv loading
import asyncio
import base64
import contextlib
import io
...
def _asset_path(name: str) -> Path:
    candidates = [
        REPO_ROOT / "server" / "codescribe" / "assets" / name,
            ...
            try:
                ...
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        "Install it (brew install ffmpeg) or upload WAV/FLAC audio."
                    ),
                ) from exc
...
async def set_model(variant: str = Body(..., embed=True)):  # noqa: B008  # FastAPI pattern
    ...
    try:
        ...
        # Justification: _sanitize_log() removes \n \r \t \x00 to prevent log injection
        logger.info(
            "Whisper model switched to: %s at %s",
            _sanitize_log(variant),
            _sanitize_log(normalized_path),
        )  # nosemgrep
...
def _fallback_sentences(topic: str, count: int = 5) -> list[str]:
    ...
    return [f"Krótkie zdanie kalibracyjne #{i + 1} dotyczące {topic_slug}." for i in range(count)]

...
struct HoldMenuItems {
    ctrl: CheckMenuItem,
    ctrl_opt: CheckMenuItem,
    ctrl_shift: CheckMenuItem,
    ctrl_cmd: CheckMenuItem,
    label: MenuItem,
}

/// Toggle Trigger menu items for radio-button behavior
struct ToggleMenuItems {
    double_opt: CheckMenuItem,
    double_ralt: CheckMenuItem,
...
thread_local! {
    ...
    static HOLD_MENU_ITEMS: RefCell<Option<HoldMenuItems>> = const { RefCell::new(None) };
    static TOGGLE_MENU_ITEMS: RefCell<Option<ToggleMenuItems>> = const { RefCell::new(None) };
    static HISTORY_MENU_ITEMS: RefCell<Option<HistoryMenuItems>> = const { RefCell::new(None) };
}
...
static HISTORY_UPDATE_CHANNEL: OnceLock<Sender<String>> = OnceLock::new();
...
fn apply_history_label_update(label_text: &str) {
...
fn apply_hold_mods_selection(mods: HoldMods) {
...
/// Must be called on main thread
fn apply_toggle_trigger_selection(trigger: ToggleTrigger) {
...
fn build_menu() -> Result<(Menu, MenuIds)> {
    ...
    // Store hold menu items for dynamic updates (main thread only)
    HOLD_MENU_ITEMS.with(|items_cell| {
    ...
    // Store toggle menu items for dynamic updates (main thread only)
    TOGGLE_MENU_ITEMS.with(|items_cell| {
    ...
    // Store history menu items for dynamic updates (main thread only)
    HISTORY_MENU_ITEMS.with(|items_cell| {
    ...
    // 10. Tools submenu (Voice Lab, Teacher, New Conversation)
...
fn handle_menu_event(event_id: &MenuId, menu_ids: &MenuIds) {
    ...
    // Tools submenu
        ...
        // Reset conversation session (clears previous_response_id)
...
pub fn run_with_hotkeys(hotkey_manager: Option<crate::hotkeys::HotkeyManager>) -> Result<()> {
    ...
    // Create channel for history label updates (from async tasks to main thread)
    ...
    event_loop.run(move |_event, _, control_flow| {
        ...
        *control_flow = ControlFlow::WaitUntil(Instant::now() + poll_interval);
        ...
        if let Ok(label_text) = history_rx.try_recv() {
            apply_history_label_update(&label_text);
        }

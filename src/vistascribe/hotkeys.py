...
from .event_log import log_hotkey_event
...
_last_assistive_mode = False  # Track assistive mode between down/up events
_non_modifier_keys_down: set[int] = set()
...
def _is_modifier_keycode(code: int) -> bool:
    return code in {
        HOLD_VK,
        KEYCODE_LEFT_OPTION,
...
def events():
    """Returns the queue used for hotkey events.

    Each entry is a tuple with at least two elements `(kind, action)`.
    Newer emitters include an optional third `assistive` flag; consumers should
    handle both tuple shapes for backwards compatibility.
    """
    return _queue
...
def set_hold_mods(spec: str) -> None:
    ...
    global _required_hold_mask
    _required_hold_mask = _parse_hold_mods(spec)
    _reset_hold_runtime_state()
...
def set_toggle_trigger(trigger: str) -> None:
    global _TOGGLE_TRIGGER, _last_alt_down_ts
    normalized = (trigger or "").strip().lower()
    if normalized not in {"double_option", "double_ralt", "none"}:
        normalized = "double_option"
    _TOGGLE_TRIGGER = normalized
    _last_alt_down_ts = 0.0
...
def _cancel_hold(reason: str | None = None) -> None:
    """Force a hold "up" event when exclusivity is violated."""

    global _last_combo_down, _block_hold_until_clear
    if _last_combo_down:
        logger.info(
            "Hotkey: Hold up%s",
            f" ({reason})" if reason else "",
        )
        _queue.put(("hold", "up", _last_assistive_mode))
        _log_hotkey("hold", "up", assistive=_last_assistive_mode)
        _last_combo_down = False
    _block_hold_until_clear = True
...
def _tap(_proxy, type_, event, _refcon):
    ...
    try:
        ...
        if type_ == Quartz.kCGEventFlagsChanged:
            ...
            modifiers_active = ctrl_is_down or alt_is_down or shift_is_down or cmd_is_down

            global _block_hold_until_clear
            if not modifiers_active and not _non_modifier_keys_down:
                _block_hold_until_clear = False
            ...
            # Special handling for CTRL vs CTRL+SHIFT distinction
            # In exclusive mode, allow Shift as an optional assistive modifier
            if _exclusive_mode:
                # Check base combo without Shift
                base_mask = present_mask & ~SHIFT_MASK
                combo_now = base_mask == _required_hold_mask
            else:
                combo_now = (present_mask & _required_hold_mask) == _required_hold_mask

            global _last_combo_down, _last_assistive_mode
            if _block_hold_until_clear or _non_modifier_keys_down:
                combo_now = False
                if _non_modifier_keys_down and _last_combo_down:
                    logger.debug(
                        "Hold cancelled because other keys are pressed: %s",
                        sorted(_non_modifier_keys_down),
                    )
                    _cancel_hold("other key pressed")
            if combo_now != _last_combo_down:
                # Determine assistive mode
                if combo_now:
                    # "down" event - check if Shift is currently pressed
                    is_assistive = shift_is_down
                    _last_assistive_mode = is_assistive  # Remember for "up" event
                else:
                    # "up" event - use remembered assistive mode (ignore current Shift state)
                    # User might release Shift before Ctrl - we want to preserve the mode
                    is_assistive = _last_assistive_mode

                _queue.put(("hold", "down" if combo_now else "up", is_assistive))
                _log_hotkey("hold", "down" if combo_now else "up", assistive=is_assistive)
                _last_combo_down = combo_now
                logger.info(
                    "Hotkey: Hold %s (mods=%s, assistive=%s, shift_currently=%s)",
            ...
            # NOTE: This works independently of hold gesture - different timing!
            # Hold = long press Ctrl+Opt, Toggle = quick double-tap Opt
            global _last_alt_state, _last_alt_down_ts
            if _TOGGLE_TRIGGER in {"double_option", "double_ralt"}:
                    ...
                    if _TOGGLE_TRIGGER == "double_ralt" and keycode != KEYCODE_RIGHT_OPTION:
                        ...
                        if alt_is_down:
                            if ctrl_is_down or shift_is_down or cmd_is_down:
                                ...
                                _log_hotkey("toggle", "press", trigger=_TOGGLE_TRIGGER)
        ...
        # KeyDown based toggles
            ...
            if not _is_modifier_keycode(keycode):
                _non_modifier_keys_down.add(keycode)
                if _last_combo_down:
                    _cancel_hold("combo interrupted by key press")
            ...
            if (
                ...
                if (flags & TOGGLE_FL) == TOGGLE_FL:
                    _queue.put(("toggle", "press"))
                    _log_hotkey("toggle", "press", trigger="slash")
                    logger.info("Hotkey: Toggle press (Shift+Cmd+/)")

        elif type_ == Quartz.kCGEventKeyUp:
            if not _is_modifier_keycode(keycode):
                _non_modifier_keys_down.discard(keycode)
                if not _non_modifier_keys_down and not (
                    flags & (CTRL_MASK | ALT_MASK | SHIFT_MASK | CMD_MASK)
                ):
                    _block_hold_until_clear = False

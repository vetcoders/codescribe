...
class Recorder:
    ...
    def __del__(self):
        """Defensive cleanup: ensure stream is closed when Recorder is garbage collected."""
        try:
            if self._stream:
    ...
    async def stop(self) -> str | None:
        ...
        try:
            ...
            self._last_duration = len(wav_data) / SR if len(wav_data) else 0.0
            logging.info(
    ...
    def snapshot_wav(self, min_seconds: float = 0.8) -> str | None:
        ...
        try:
            if not self._buf:
                return None
            total_frames = int(sum(len(b) for b in self._buf))
            if total_frames < int(SR * min_seconds):
                return None
            wav_data = np.concatenate(self._buf)
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tf:
                path = tf.name

...
//!
//! Reached as `codescribe corpus` and, for compatibility, as the standalone
//! `codescribe-corpus` binary. Two properties survive that move only because
//! they are handled explicitly:
//!
//! - the Keychain lock. The legacy binary set `CODESCRIBE_DISABLE_KEYCHAIN`
//!   as its first executable statement, before Clap ran. A subcommand cannot
//!   be first, so `bin/codescribe.rs` peeks raw argv and sets it before
//!   parsing — see [`corpus_argv_requested`].
//! - the self-spawn. The matrix runner re-invokes `current_exe()` with
//!   `worker`. Under the unified entry point that executable is `codescribe`,
//!   so the child needs `corpus worker`. The prefix travels in
//!   [`Invocation`] rather than being guessed from argv at spawn time.
//!
...
use crate::controller::production_replay::{ProductionReplayLane, replay_overlay_recording};
...
/// Raw-argv test for the corpus lane, used before Clap has run.
///
/// The Keychain lock must be in place before any parsing, so the decision
/// cannot wait for a typed command. It accepts only an exact `corpus` in the
/// first position: no abbreviation, no flag, so nothing else can trip it.
pub fn corpus_argv_requested() -> bool {
    corpus_argv_requested_in(std::env::args_os())
}

/// The decision above, over an explicit argv, so it can be tested without a
/// process. A guard that cannot be tested is a guard nobody can trust.
pub fn corpus_argv_requested_in(argv: impl IntoIterator<Item = std::ffi::OsString>) -> bool {
    argv.into_iter().nth(1).is_some_and(|word| word == "corpus")
}

/// How this process was entered, so a self-spawned worker is addressed the way
/// its parent was.
#[derive(Debug, Clone, Default)]
pub struct Invocation {
    /// argv words that must precede `worker` when re-invoking `current_exe()`.
    /// Empty for the legacy binary; `["corpus"]` under `codescribe corpus`.
    prefix: Vec<String>,
}

impl Invocation {
    /// The legacy `codescribe-corpus` binary: `worker` is argv[1].
    pub fn legacy_binary() -> Self {
        Self { prefix: Vec::new() }
    }

    /// The unified `codescribe corpus` subcommand: `worker` is argv[2].
    pub fn subcommand(word: &str) -> Self {
        Self {
            prefix: vec![word.to_string()],
        }
    }
}
...
/// Run a corpus command and map the outcome onto the tool's exit contract.
///
/// The caller is responsible for having set `CODESCRIBE_DISABLE_KEYCHAIN`
/// before any parsing; see the module docs.
pub fn main_with(command: CorpusCommand, invocation: Invocation) -> ExitCode {
    match run(command, invocation) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
...
fn run(command: CorpusCommand, invocation: Invocation) -> Result<()> {
    match command {
        CorpusCommand::Compare {
            baseline,
            candidate,
        } => {
...
fn run_matrix(args: MatrixArgs, invocation: &Invocation) -> Result<()> {
    ...
    for profile in &args.profiles {
        ...
        // Under `codescribe corpus` the executable is the unified binary, so
        // the child must be addressed the same way this process was.
...
fn success_quality_entry(
    ...
    replay: &crate::controller::production_replay::ProductionOverlayReplay,
) -> ReportEntry {
...
mod invocation_tests {
    ...
    #[test]
    fn the_keychain_lock_arms_only_for_the_corpus_lane() {
        assert!(corpus_argv_requested_in(argv(&[
            "codescribe",
            "corpus",
            "census"
        ])));
        assert!(corpus_argv_requested_in(argv(&["codescribe", "corpus"])));
        // Any other lane must NOT disable the Keychain: transcribe and the
        // agent surfaces legitimately need it.
        assert!(!corpus_argv_requested_in(argv(&[
            "codescribe",
            "transcribe",
            "a.wav"
        ])));
        assert!(!corpus_argv_requested_in(argv(&["codescribe"])));
        // No abbreviation, no flag spelling, nothing positional further along.
        assert!(!corpus_argv_requested_in(argv(&["codescribe", "corp"])));
        assert!(!corpus_argv_requested_in(argv(&["codescribe", "--corpus"])));
        assert!(!corpus_argv_requested_in(argv(&[
            "codescribe",
            "report",
    ...
    #[test]
    fn a_self_spawned_worker_is_addressed_the_way_its_parent_was() {
        // The matrix runner re-invokes `current_exe()`. Under the legacy binary
        // that executable takes `worker` directly; under the unified one it
        // takes `corpus worker`, or the child parses a command that does not
        // exist and the whole matrix fails at the first profile.
        assert!(Invocation::legacy_binary().prefix.is_empty());
        assert_eq!(Invocation::subcommand("corpus").prefix, vec!["corpus"]);
    }
}

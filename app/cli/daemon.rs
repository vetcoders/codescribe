...
//!
//! Reached as `codescribe daemon` and, for compatibility, as the standalone
//! `qube-daemon` binary.
//!
//! Usage:
//!   codescribe daemon --date 2026-01-17 --apply
//!   codescribe daemon --daemon   # background loop, 1h interval
...
use crate::qube_daemon::{LexiconSource, QubeDaemonConfig, run as run_quality_loop};
...
/// Job entry point: apply env overrides, then branch to daemon or single-run mode.
///
/// The `set_var` calls sit at the very top, before any await, because that is
/// the only point where the process is still single-threaded — the safety
/// contract those `unsafe` blocks document. Reaching this through
/// `codescribe daemon` preserves that: the subcommand dispatch does no work
/// before calling in.
...
async fn run_single(args: &DaemonArgs) -> Result<()> {
...
async fn run_single_with_transcription(
    ...
    let out = run_quality_loop(loop_config).await?;
    ...
    loop {
        ...
        match run_single_with_transcription(&check_args, LocalTranscriptionMode::LocalWhisper).await
        {
            Ok(()) => {
                ...
                if let Err(err) = crate::qube_daemon::write_daemon_state(mismatches) {
...
fn build_daemon_check_args(args: &DaemonArgs, date_filter: String) -> DaemonArgs {
    DaemonArgs {
...
mod tests {
    ...
    /// Baseline `DaemonArgs` for daemon unit tests (non-daemon defaults).

...
//! ```
//!
//! Reached as `codescribe teach` and, for compatibility, as the standalone
//! `codescribe-teacher` binary.
//!
//! Examples:
//! ```bash
//! # Built-in proof (e2e 01_no-to-dobra texts)
//! codescribe teach proof
//!
//! # Custom files
//! codescribe teach compare \
...
/// Assemble a [`TeacherInput`] from the chosen source and hand it to [`report`].
pub fn run(cmd: TeacherCommand) -> Result<()> {
    match cmd {
        TeacherCommand::Proof { html, json } => {
            ...
            report(input, html, json)
        }
        TeacherCommand::Compare {
            live,
            whisper,
            human,
            label,
...
/// Run the teacher and print its findings; optionally emit JSON and an HTML file.
///
...
fn report(input: TeacherInput, html: Option<PathBuf>, json: bool) -> Result<()> {

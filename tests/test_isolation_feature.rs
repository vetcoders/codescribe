#[test]
fn root_integration_test_refuses_account_home_lock_without_runtime_arming() {
    let home = codescribe_core::test_isolation::account_home().expect("passwd account home");
    let path = home.join(format!(
        ".codescribe/test-isolation-root-{}.lock",
        std::process::id()
    ));
    assert!(!path.exists(), "probe path must be absent");
    let result = std::panic::catch_unwind(|| {
        let _ = codescribe_core::config::acquire_agent_turn_lease_at(&path);
    });

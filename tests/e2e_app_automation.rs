...
fn e2e_app_automation_commands_drive_native_surface() {
    ...
    assert!(!initial.settings_visible);
    ...
    let settings = parse_state(run_app_command(
        &data_dir,
        &["app", "action", "show-settings"],
    ));
    assert!(settings.settings_visible);

    let hidden_settings = parse_state(run_app_command(
        &data_dir,
        &["app", "action", "hide-settings"],
    ));
    assert!(!hidden_settings.settings_visible);

    let dock_reopen = parse_state(run_app_command(
        &data_dir,
        &["app", "action", "trigger-dock-reopen"],
    ));
    assert!(dock_reopen.settings_visible);

    let reset = parse_state(run_app_command(&data_dir, &["app", "action", "reset-ui"]));
    assert!(!reset.settings_visible);

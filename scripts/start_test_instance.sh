#!/bin/bash
#!/bin/bash
# Start test instance of CodeScribe on port 7237 with refactored code
...
export CODESCRIBE_INSTANCE=test
export INSTANCE_LOCK_FILE=.codescribe_test.lock
export DEV_MODE=1
export NOHUP_MODE=1
...
export CODESCRIBE_PORT_FALLBACKS=${CODESCRIBE_PORT_FALLBACKS:-"7237,6237,5237"}

# Start backend on port 7237
echo "Starting backend on port 7237..."
nohup uv run python -m codescribe.codescribe_server > logs/test-server.log 2>&1 &
echo "Backend PID: $!"
sleep 2

# Start the tray using the packaged module path
echo "Starting tray (codescribe.main) with test config..."
nohup uv run python -m codescribe.main > logs/test-main.log 2>&1 &
echo "Main PID: $!"
...
echo "   pkill -f 'CODESCRIBE_INSTANCE=test'"

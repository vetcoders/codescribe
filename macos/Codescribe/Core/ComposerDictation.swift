...
final class RealComposerDictation: ComposerDictating {
  ...
  private func start(autoSend: Bool) {
    ...
    Task { @MainActor in
      ...
      do {
        ...
        _ = hotkeys.setAgentCaptureActive(active: false)
        dictationLog.error(
          "composer dictation start failed: \(error.localizedDescription, privacy: .public)")
  ...
  private func stop() {
    ...
    Task { @MainActor in
      ...
      do {
        ...
        dictationLog.error(
          "composer dictation stop failed: \(error.localizedDescription, privacy: .public)")
        store.clearDictationPreview()
  ...
  private func handleEngineError(message: String) {
    ...
    guard let store else { return }
    guard transitioning || store.dictationPhase == .preparing || store.dictationPhase == .recording
    else { return }

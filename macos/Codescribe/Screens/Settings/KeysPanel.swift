...
struct KeysPanel: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      Text(
        "Stored in the macOS Keychain. Keys are write-only here — codescribe never displays a stored secret."
      )
      .font(CSFont.ui(12.5))
      .lineSpacing(2)
...
struct LLMLanesSection: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 14) {
      Text(
        "Set provider, endpoint, and model per request path. Leave an override empty to use the resolved fallback."
      )
      .font(CSFont.ui(11.5))
      .lineSpacing(2)
      .foregroundStyle(CSColor.textMutedAlt)
...
/// Editor for `STT_ENDPOINT` — the live WSS socket mapper (Libraxis multipart
/// URL → `wss://…/v1/audio/transcribe`). The API key stays in the Keychain
/// on Providers. Persist key is unchanged.
struct SttEndpointRow: View {
  let current: String
...
/// Non-secret URL field used for the Cloud session-mint endpoint.

...
struct MarkdownText: View {
  ...
  private func blockView(_ block: MDBlock, isLast: Bool) -> some View {
    ...
    case .bullet(let indent, let text):
      listRow(
        marker: bulletMarker(indent), indent: indent, text: text,
        isLast: isLast, deep: indent >= 2)
    case .ordered(let indent, let number, let text):
      listRow(
        marker: "\(number).", indent: indent, text: text,
  ...
  private func inlineText(
    _ text: String, baseFont: Font, baseColor: Color,
    fontSize: CGFloat, isLast: Bool
  ) -> some View {
  ...
  private func listRow(
    marker: String, indent: Int, text: String, isLast: Bool,
    deep: Bool
  ) -> some View {
  ...
  private func tableView(header: [String], rows: [[String]]) -> some View {
    ...
    MDTableLayout(columns: columnCount, rowCount: rows.count + 1, weights: weights) {
      ...
      ForEach(Array(rows.enumerated()), id: \.offset) { index, row in
        ForEach(0..<columnCount, id: \.self) { column in
          tableCell(
            cell(row, column), isHeader: false,
  ...
  private func codeBlock(_ language: String?, _ content: String, isLast: Bool) -> some View {
    ...
    let block = CodeBlockView(
      content: content, language: language, size: s,
...
enum CalloutKind {
  ...
  static func detect(_ raw: String) -> (kind: CalloutKind, body: String)? {
    ...
    guard
      let head = lines.firstIndex(where: {

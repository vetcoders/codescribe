...
final class AgentThreadTitleTests: XCTestCase {
  ...
  func testEarlyTitleUpdatesOriginalThreadWithoutChangingSelection() async throws {
    ...
    await waitUntil {
      store.threads.first(where: { $0.id == first.id })?.title == "Race-proof Swift titles"
    }
  ...
  func testMissingFirstPersistRetriesExactlyOnceBeforeRefresh() async {
    ...
    await waitUntil {
      provider.events.filter { $0 == .generated("Title state machine") }.count == 1
    }
    ...
    XCTAssertEqual(
      store.currentThread?.title, "Title state machine",
      "refresh must not flash the heuristic slug back")
  ...
  func testFirstVoiceExchangeLaunchesExactlyOneTitleRequestFromPresentedText() async {
    ...
    XCTAssertEqual(
      engine.titleCalls, ["Plan the voice rename race"],
      "title must use presented text, not the wire skeleton")
  ...
  func testSecondVoiceExchangeLaunchesNoAdditionalTitleRequest() async {
    ...
    XCTAssertEqual(
      engine.titleCalls.count, 1,
      "later exchanges on the same backend thread launch no title request")
    XCTAssertTrue(engine.streamCalls.isEmpty)
  }
  ...
  func testVoiceEarlyTitleQueuesAndFlushesAfterCorePersistenceOnDone() async {
    ...
    XCTAssertEqual(
      provider.events.filter { $0 == .generated("Voice title state") }.count, 2,
    ...
    XCTAssertNotNil(refresh)
    XCTAssertLessThan(
      lastGenerated!, refresh!, "queued voice title must flush before the rail refresh")
    XCTAssertEqual(store.currentThread?.title, "Voice title state")
    XCTAssertEqual(
      store.currentThread?.backendId, provider.backendID,
      "selection stays on the same backend thread")
  }
  ...
  func testVoiceManualRenameAfterGeneratedTitleWinsPermanently() async {
    ...
    store.ingestVoiceTurn(
      threadId: provider.backendID, userText: "voice late generated then rename")
    selectVoiceThread(provider.backendID, in: store)
    await waitUntil { engine.titleCalls.count == 1 }
    provider.markFirstTurnPersisted()
    store.ingestVoiceDone()
  ...
  func testVoiceStopBeforeTitleCompletionPreservesFallbackAndPersistsNothing() async {
    ...
    XCTAssertEqual(
      store.currentThread?.title, "voice cancel race",

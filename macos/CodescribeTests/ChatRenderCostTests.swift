...
final class ChatRenderCostTests: XCTestCase {
  ...
  private func streamingThread(streamChars: Int, pastedChars: Int? = nil) -> [ChatMessage] {
    ...
    if let pastedChars {
      messages.append(
        ChatMessage(role: .you, timestamp: "now", text: pastedProse(chars: pastedChars)))
    }
    messages.append(
      ChatMessage(role: .you, timestamp: "now", text: "where do we double-dispatch events?"))
  ...
  private func legacyMarkdownTick(_ text: String) -> Int {
    ...
    var materialized = 0
    for block in blocks {
      if case .paragraph(let body) = block {
  ...
  private func perTickMicros(_ label: String, body: () -> Int) -> Double {
    ...
    print(
      "[render-cost] \(label): \(String(format: "%.1f", micros)) µs/tick (over \(ticks) ticks, sink \(sink))"
    )

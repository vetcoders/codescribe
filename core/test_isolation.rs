...
pub fn assert_test_write_allowed(path: &Path) {
    ...
    let Some(home) = account_home() else {
        panic!("test isolation cannot determine the account home before writing {}", path.display());
    };
    let absolute = if path.is_absolute() {
        path.to_path_buf()
    } else {
        std::env::current_dir().expect("test isolation current directory").join(path)
    };
    ...
    if normalized.starts_with(&home) {
        panic!("test process refused write under real home: {}", path.display());
    }
}
...
mod tests {
    ...
    fn real_home_mutations_panic_but_reads_and_temp_writes_do_not() {
        ...
        for operation in ["write", "create_dir_all"] {
            ...
            assert!(outcome.is_err(), "{operation} must be refused before mutation");
        }

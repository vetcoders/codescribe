...
/// Supervisor-mode Silero at the Apple PCM ingress.
...
impl SileroIngress {
    ...
    pub fn ingest(&mut self, samples: &[f32], samples_seen: u64) -> Vec<u64> {
        let mut closed = Vec::new();
        if samples.is_empty() {
            return closed;
        }
        let events = self.vad.feed(samples, 0);
        if let Some((start, end)) = self.vad.open_segment_raw_range() {
            self.ledger
                .open_or_extend(&self.session, self.capture_epoch, start, end);
        }
        for event in events {
            if matches!(event, SpeechEvent::UtteranceFinal(_))
...
mod tests {
    ...
    fn lane_defaults_off() {
        // The process may inherit the operator dotenv; the parser itself must
        // treat unset as off. Pin the env for this test.

...
mod tests {
    ...
    /// 2026-08-14: the flag became the off-switch. Unset must arm the lane, and
    /// only the explicit off tokens may return the session to Apple boundaries.
    #[test]
    fn lane_defaults_on_and_flag_is_the_off_switch() {
        // The process may inherit the operator dotenv; pin the env for this test.
        let previous = std::env::var(SILERO_FUSION_ENV).ok();
        unsafe { std::env::remove_var(SILERO_FUSION_ENV) };
        assert!(lane_enabled(), "unset must arm the lane");
        for off in ["0", "false", "no", "off", " OFF "] {
            unsafe { std::env::set_var(SILERO_FUSION_ENV, off) };
            assert!(!lane_enabled(), "{off:?} must disarm the lane");
        }
        for on in ["1", "true", "yes", "on"] {
            unsafe { std::env::set_var(SILERO_FUSION_ENV, on) };
            assert!(lane_enabled(), "{on:?} must keep the lane armed");
        }
        match previous {
            Some(value) => unsafe { std::env::set_var(SILERO_FUSION_ENV, value) },
            None => unsafe { std::env::remove_var(SILERO_FUSION_ENV) },
        }
    }

...
- `FINAL_PASS_MODE` / `CODESCRIBE_FINAL_PASS_MODE` — retired, ignored at runtime and rejected on config writes; repair removes the persisted setting. Retranscribe remains explicit.
- `CODESCRIBE_LAYERED_TRANSCRIPTION` — env-only diagnostic override for Local Power. Unset arms by mode; `phase1`/`1` arms explicitly; off or invalid input degrades. Settings displays the override without writing it. Cloud admission ignores it.

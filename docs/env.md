...
- `CODESCRIBE_WHISPER_IDLE_UNLOAD_SECS` (HOT RELOADED dla wartości progu; default `60` = 1 min; `0` jest jawnym keep-warm — włączenie reaper'a z `0` wymaga restartu) — po N s bezczynności silnik Whisper jest zwalniany z pamięci (GPU/host) i ładowany ponownie przy następnym użyciu. Każdy cold-load loguje `whisper_residency_load`, a reaper `whisper_residency_reclaim`: zdarzenia INFO mają efektywny TTL, liczniki oraz czasy load/unload/reclaim, lecz nigdy audio ani treść transkrypcji. Pełny fp16 model jest ścieżką produktu, więc następny cold-load nie płaci już każdorazowej dekwantyzacji q8→F32. `0` kupuje zerową latencję kosztem trzymanego RSS.

...
**STT endpointy**
- `STT_ENDPOINT` > `WHISPER_SERVER_URL` (RESTART NEEDED)
...
- `LLM_ENDPOINT` jest canonical; `LLM_SERVER_URL` to legacy (HOT RELOADED)
- `LLM_HOST` > `OLLAMA_HOST` (dla lokalnego hosta) (HOT RELOADED)
- `LLM_MODEL` > `OLLAMA_MODEL` (HOT RELOADED)
...
- `STT_ENDPOINT`, `STT_API_KEY`, `WHISPER_SERVER_URL` (RESTART NEEDED)
...
- `AI_PROVIDER` (legacy) (RESTART NEEDED)
- `LLM_ENDPOINT`, `LLM_MODEL`, `LLM_API_KEY` (HOT RELOADED)
...
- `LLM_HOST`, `LLM_SERVER_URL` (legacy) (HOT RELOADED)
- `OLLAMA_HOST`, `OLLAMA_MODEL` (legacy) (HOT RELOADED)

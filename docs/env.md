...
- `STT_TAIL_PROVIDER` (HOT RELOADED; default `inprocess`) — wybiera implementację kontraktu tail-patch. W W13-2A uzbrojone jest wyłącznie `inprocess`; `sidecar` i `remote` są poprawnymi wartościami kontraktu, ale fail-closed do czasu hostingu W13-2B. Zmiana defaultu jest guzikiem operatora.

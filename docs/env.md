...
- `CODESCRIBE_LAYERED_TRANSCRIPTION` (HOT RELOADED; default `phase1`; `phase1`..`phase4` lub bare `1`..`4`; jawne `off`/`0`/`false` rozbraja) — legacy gate lokalnego Layer 1 Whisper tail-patch (`ReplaceRange`) na obu ścieżkach live. Jest skuteczny **wyłącznie** przy resolved `CODESCRIBE_ASR_MODE=local_power`; cloud i apple_only niosą typed boundary „zero lokalnych wag”, którego env nie może poszerzyć. Promoted do `settings.json`. Smart final-pass nie ustawia tej flagi.

...
- `CODESCRIBE_LAYERED_TRANSCRIPTION` (HOT RELOADED; default `off`; jawne `phase1` lub bare `1` uzbraja eksperymentalną lane) — ortogonalny gate warstwowej transkrypcji. Unset zachowuje Apple live + lexicon bez mutacji Layer 1. `phase1` uruchamia Whisper tail-patch (`ReplaceRange`) na obu ścieżkach live; pozostaje opt-in do czasu domknięcia PCM/span identity, jednego rewrite fence, strukturalnej idempotencji i pełnych receiptów stop-drain. Promoted do `settings.json`. Smart final-pass **nie** ustawia tej flagi.

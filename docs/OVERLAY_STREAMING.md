...
| ------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------- | ----------------------------------------------------------------- |
| **0 — Live**        | Apple `SFSpeechRecognizer` (primary); local recovery only in `local_power` | `Preview`, `Correction`, `UtteranceFinal`                            | While the user speaks — owns first commit | ✅ shipped, default                                               |
| **1 — Tail Patch**  | authorized cloud provider, or local Whisper only in `local_power`          | `ReplaceRange { source: TailPatch }`                                 | at each sealed utterance boundary         | ✅ delivered; product-mode constrained                            |

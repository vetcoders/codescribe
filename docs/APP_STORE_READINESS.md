...
| 2   | **Developer ID + notarization** is the _outside-the-store_ lane; the store lane needs an **Apple Distribution / "3rd Party Mac Developer"** signing identity, a `.pkg` (`productbuild`), and an App Store Connect upload (Transporter).                                                                                                                                    | [Notarizing macOS software](https://developer.apple.com/documentation/security/notarizing-macos-software-before-distribution)                                                                                                                                                                                            |
...
| 7   | **Input Monitoring** (CGEventTap _listen-only_, via `CGPreflightListenEventAccess` / `CGRequestListenEventAccess`) **is available to sandboxed Mac App Store apps**. Global-hotkey _detection_ can survive sandbox; _controlling other apps_ cannot.                                                                                                                       | [Xojo: Sandboxing to Notarization](https://blog.xojo.com/2024/08/22/macos-apps-from-sandboxing-to-notarization-the-basics/), [Beyond App Sandbox](https://www.appcoda.com/mac-app-sandbox/)                                                                                                                              |
...
| **Privacy manifest**       | **Superseded call-site inventory from the dated release audit:** `PrivacyInfo.xcprivacy` was absent and the audit found `std::fs` `metadata().modified()` in `core/state/history.rs`, `core/hf_cache.rs`, `core/attachment.rs`, plus the then-live `core/pipeline/stream_postprocess.rs`. That last surface has since been deleted and is not current evidence. C8A makes no new readiness verdict: a release lane must re-prove every live required-reason API call-site before selecting a category/reason. | Re-prove against current live call-sites, then declare every applicable required-reason API in `PrivacyInfo.xcprivacy` | **Historical P0; current call-site proof NOT ASSESSED by C8A** (draft template: `scripts/PrivacyInfo.xcprivacy.template`) |
...
_third-party SDKs on Apple's commonly-used list_, but the required-reason
_declaration_ obligation also covers an app's own first-party usage of those
...
the store) and un-sandboxed (for the agent). What it changes is the _action_: stop
...
and library validation is satisfied _and_ the sandbox's "nested code must be
...
uses the runtime _Input Monitoring_ privilege, not the Accessibility privilege that
...
Basic SKU unchanged**. What does _not_ carry is _controlling other apps_ (paste,
...
- **Historical re-fire receipt, superseded as current call-site inventory:**
  the then-live `stream_postprocess.rs` was an additional FileTimestamp site
  the first draft missed. The file was later deleted; retain the finding as
  release archaeology, but re-prove the current privacy-manifest requirement
  from live call-sites in a release lane.
...
These are the _documented target commands_, not run by this pass (they need an

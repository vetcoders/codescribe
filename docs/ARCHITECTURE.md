...
| **L2 — Lexicon + Light+**    | Deterministic authorized relabeling    | `admit_ledger_label` records the Lexicon observation; `core/pipeline/light_plus.rs` (restored 2026-09-08 on the Founder's word) mints one `light-plus` document revision at the terminal seal via `PresentationEmitter::mint_light_plus_revision`, skipped only for the literal Ctrl-hold take; `custom_lexicon_entries` is the persisted custom-lexicon loading path |
